...
class PoseEstPostProcessing:
    ...
    def fast_post_process(self, raw_detections: dict, height: int, width: int) -> List[np.ndarray]:
        """
        Efficiently post-processes a single output layer for pose estimation.
        Groups 17 keypoints into individual person detections based on average confidence.

        Args:
            raw_detections (dict): Raw output from Hailo (expects only one keypoint layer of shape (1, 80, 80, 51)).
            height (int): Input image height.
            width (int): Input image width.

        Returns:
            List[np.ndarray]: List of poses (each pose is (17, 3) array with x, y, confidence).
        """
        # Find the keypoints tensor
        for output in raw_detections.values():
            if output.shape == (1, 80, 80, 51):
                keypoints_tensor = output
                break
        else:
            raise ValueError("Expected (1, 80, 80, 51) keypoint layer not found.")

        stride = 8
        num_kpts = 17
        grid_h, grid_w = 80, 80

        # Reshape to (1, 6400, 17, 3)
        keypoints = np.reshape(keypoints_tensor, (1, grid_h * grid_w, num_kpts, 3))

        # Generate grid centers (6400, 1, 2)
        grid_y, grid_x = np.meshgrid(np.arange(grid_h), np.arange(grid_w), indexing='ij')
        center = np.stack([(grid_x + 0.5) * stride, (grid_y + 0.5) * stride], axis=-1)  # (80, 80, 2)
        center = center.reshape(1, grid_h * grid_w, 1, 2)

        # Convert normalized keypoints to image coordinates
        keypoints[..., :2] *= 2
        keypoints[..., :2] = stride * (keypoints[..., :2] - 0.5) + center  # (1, 6400, 17, 2)

        # Get average confidence per 17-joint group (1, 6400)
        pose_confidence = keypoints[..., 2].mean(axis=2)

        # Apply pose threshold
        mask = pose_confidence[0] > self.score_threshold
        poses = keypoints[0][mask]  # shape: (N poses, 17, 3)
    ...
    # ) -> np.ndarray:
        # """
        # Draw confident keypoints on the image.

        # Args:
            # keypoints (np.ndarray): Array of shape (N, 3) with x, y, confidence.
    ...
    def extract_pose_estimation_results(
        ...
        """
        #print(f'TIMEEE: {time.perf_counter()}')
        ...
        #print(f'TIMEEE: {time.perf_counter()}') # HIGH EXECUTION TIME IN BELOW BLOCK
